 ...
 */
function buildPlanetGeometry(radius) {
    ...
    for (let i = 0; i < pos.count; i++) {
        ...
        const wobble = Math.sin(lon * 2.2 + lat * 1.5) * 0.5 +
...
export function Globe({ children }) {
    ...
    const planetMat = useMemo(() => new THREE.MeshStandardMaterial({ vertexColors: true, flatShading: true, roughness: 0.92, metalness: 0 }), []);
    ...
    return (_jsxs("group", { children: [_jsxs("group", { ref: planetRef, children: [_jsx("mesh", { geometry: geometry, material: planetMat, castShadow: true, receiveShadow: true }), _jsx(SurfaceProps, { radius: GLOBE_RADIUS }), children] }), _jsx("mesh", { material: atmoMat, scale: 1.16, children: _jsx("icosahedronGeometry", { args: [GLOBE_RADIUS, 12] }) })] }));
}
